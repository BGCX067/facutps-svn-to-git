...
bienvenida:
	...
	; Ejercicio 1
		...
		; actualizar la informacion de los registros DS ES GS FS SS
			mov ax, 0x18
			mov ds, ax			;cual de estos hay que poner en 18???????????
	...
	mov ax, 0x10		;vuelvo al segmento de datossssssssssss!!!
		mov ds, ax
		mov es, ax
		mov fs, ax
		mov gs, ax
...
;------------------------------------PAGINACION PINTOR-------------------------------------------------------
	fillPintorTablePages:				;inicializo la primer entrada del directorio con la direccion de la tabla
		mov 	esi,pintorTable	;porque se que tengo que llenar la primer tabla
		mov 	eax, 0
		mov ecx,9 			; 1024 = 4m
		llenarTablas:
			or 	eax, 0x3		;supervisor, read/write, present
	...
	mov 	eax, 0
	lea esi,[esi + 4 * 14]	
	lea eax,[eax + 4096 * 14]	;SALTEO LOS QUE NO ESTAN EN ID MAPPING
	or 	eax, 0x3		;supervisor, read/write, present
	mov 	[esi], eax
	lea esi,[esi + 4]	;porque cada descriptor de tabla(eax) tiene 4 bytes
	lea eax,[eax + 4096];porque cada tabla ocupa 4k.
	...
	mov 	eax, 0
	lea esi,[esi + 4 * 21]	
	lea eax,[eax + 4096 * 21]	;SALTEO LOS QUE NO ESTAN EN ID MAPPING	
	or 	eax, 0x3		;supervisor, read/write, present
	mov 	[esi], eax
...
fillTyKTablePages:				;inicializo la primer entrada del directorio con la direccion de la tabla
		mov 	esi,traductorYKernelTable	;porque se que tengo que llenar la primer tabla
		mov 	eax, 0
		mov ecx,8 			; 1024 = 4m
		llenarTablastYk:
			or 	eax, 0x3		;supervisor, read/write, present
...
;lleno las dir 9000
		...
		llenarTablastYk2:
			...
			lea esi,[esi + 4]	;porque cada descriptor de tabla(eax) tiene 4 bytes
			lea eax,[eax + 4096];porque cada tabla ocupa 4k.
		loop llenarTablastYk2 ;aca completamos hasta la 0x8fff
		...
		mov 	eax, 0
		lea esi,[esi + 4 * 22]	
		lea eax,[eax + 4096 * 22]	;SALTEO LOS QUE NO ESTAN EN ID MAPPING	
		or 	eax, 0x3		;supervisor, read/write, present
		mov 	[esi], eax
...
;------------------------------------FIN PAGINACION TRADUCTOR Y KERNEL-------------------------------------------------------
	...
	mov eax, traductorYKernelPage_dir		;cargo la direccion del directorio en cr3
	mov cr3, eax
...
;-------------------------EJERCICIO 2b-----------------------------------------------------------------------------------
	...
	mov ecx, mensajeEj_len		
		...
		mov esi, mensajeEj
		xor edi, edi
		add edi, (160) + 2 + 0x13000	; accedo a la memoria de video a traves del 
							; paginacionn
...
%endrep
;;;;;;;TIMES 0xB000 - KORG - ($ - $$) db 0x00 ;rellena de 0 hasta la direccion
traductorYKernelPage_dir:	
%rep	0x400
	dd	0x00000002		;supervisor, read/write, not present
%endrep
;;;;;;;TIMES 0xC000 - KORG - ($ - $$) db 0x00 ;rellena de 0 hasta la direccion
pintorTable:	
%rep	0x400
	dd	0x00000002		;supervisor, read/write, not present
%endrep
;;;;;;;TIMES 0xD000 - KORG - ($ - $$) db 0x00 ;rellena de 0 hasta la direccion
...
; 0x20(3) descriptor de TSS para la tarea
...
; 0x28(3) descriptor de TSS para la tarea
	dw 0x67			; segment limit
	dw 0x00			; base 15
	db 0x00			; base
	db 10001001b	;
...
; 0x30(3) descriptor de TSS para la tarea
	dw 0x67			; segment limit
	dw 0x00			; base 15
	db 0x00			; base
	db 10001001b	;
