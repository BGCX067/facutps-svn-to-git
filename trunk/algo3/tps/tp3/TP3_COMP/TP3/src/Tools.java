...
public class Tools {
	 ...
	 *
	 */
	public static void escribirSolucionXLS(Ejercicio ej) {
		try {
			// El archivo fileName ya esta generado, se le agregan lineas nuevas. 
			BufferedWriter out = new BufferedWriter(new FileWriter(ej.getOutFileName()+".xls", true));
			long cc = ej.getComponentesConexas()!=null? ej.getComponentesConexas().size() : 0;			
//			out.write(ej.getVertices().size() + " \t " + ej.getCantidadAristas() + " \t " + cc + " \t " +
//					ej.getSolucionGlobal().getPeso() + " \t " + Global.comp + " \t " + ej.getTiempo() + "\n");
			
			out.write(ej.getSolucionGlobal().getPeso() + "\n");
	...
	public static void escribirTablas(Ejercicio ej) {
		try {
			// El archivo fileName ya esta generado, se le agregan lineas nuevas. 
			BufferedWriter out = new BufferedWriter(new FileWriter(ej.getOutFileName()+"table.txt", true));
//			long cc = ej.getComponentesConexas()!=null? ej.getComponentesConexas().size() : 0;			
			//out.write(ej.getVertices().size() + " & " + ej.getCantidadAristas() + " & " + cc + " & " +
			//		ej.getSolucionGlobal().getPeso() + " & " + Global.comp + " & " + ej.getTiempo() + "\\\\");
			out.write( ej.getSolucionGlobal().getPeso() + " & " + Global.comp +  "\\\\");
			out.newLine();
...
//			BufferedWriter out = new BufferedWriter(new FileWriter(ejercicio.getOutFileName()+".xls"));
//			out.write("Nodos \t Aristas \t compCon \t Peso \t Comparaciones \t Tiempo \n");
//			out.close();
			
			
//			out = new BufferedWriter(new FileWriter(ejercicio.getOutFileName()+"table.txt"));
//			out.write("\\hline"); out.newLine();
//			out.write("Nodos & Aristas & compCon & Peso & Comparaciones & Tiempo \\\\");out.newLine();
//			out.write("\\hline"); out.newLine();
//			out.close();
			
//			System.out.println("Empiezo a leer el archivo..");
			// Empieza a leer el archivo
			BufferedReader in = new BufferedReader(new FileReader(Ejercicio.IN_PATH + Ejercicio.IN_FILE_NAME));
			...
			while (! line.equals("0")){
				ejercicio.setCantidadAristas(0);
				...
				for(int j=0; j < nodos ;j++){
					...
					for(int a=1; a < datos.length; a++){
						...
						ejercicio.setCantidadAristas(ejercicio.getCantidadAristas() + 1);
					}
				}
				
				ejercicio.setInicio(new Timestamp(Calendar.getInstance().getTimeInMillis()));
				
				if(!(ejercicio instanceof AlgoritmoExacto)) 
				...
				ejercicio.setFin(new Timestamp(Calendar.getInstance().getTimeInMillis()));
...
//		System.out.println("Separe las componentes conexas. " + (numero_componentes_conexas-1));
		return componentesConexas;
	}
	
	//TODO: Solo para iteraciones.. el grafico ese
	public static void escribirIteracionesXLS() {
		try {
			// El archivo fileName ya esta generado, se le agregan lineas nuevas. 
			BufferedWriter out = new BufferedWriter(new FileWriter("iteracionesGRASP.xls"));
			
			for(int i=0; i < 1000; i+=5){
				int suma=0;
				for(int j=0; j < 5; j++)
					suma += Global.iteraciones[i+j];
				
				out.write(i + " - " + (i+5) + " \t " + suma + "\n");				
			}
	...
	private static ArrayList<Vertice> quickSort(ArrayList<Vertice> vertices, int orden, int op) {
		if (vertices.size() <= 1)
			return vertices;
		int pivot = vertices.size() / 2;Global.comp++;
