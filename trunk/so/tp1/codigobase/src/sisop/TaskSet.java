...
class TaskSet implements Iterable<Task> {
    ...
    protected final List<Task> tasks = new LinkedList<Task>();
    ...
    public List<Task> get_released_tasks_at_time(int time) {
        List<Task> incoming_tasks = new LinkedList<Task>();
        for (Task task : tasks) {
            if (task.rtime == time) {
